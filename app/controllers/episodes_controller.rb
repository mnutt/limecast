class EpisodesController < ApplicationController
  ...
  def index
    ...
    @episodes = @podcast.episodes.find(:all, :include => [:podcast_slug], :order => "published_at DESC")
  ...
  def search
    @q        = params[:q]
    @podcast  = Podcast.find_by_clean_url(params[:podcast_slug])
    @episodes = @podcast.episodes.search(@q, :include => [:podcast_slug]).compact.uniq.sort_by(&:published_at)
    @feeds    = @podcast.feeds
    render :action => 'index'
  end
