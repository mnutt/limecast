  ...
  @podcasts.each do |podcast|
    xml.podcast do
      ...
      xml.primary_feed :url => podcast.primary_feed.url do
        xml.newest_episode :url => podcast.newest_source.url, :size => podcast.newest_source.size, :published_at => podcast.newest_source.episode.published_at.to_s(:rfc822) if podcast.newest_source
      end
