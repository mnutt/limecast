...
describe UserMailer do
  ...
  it 'should send a claim podcasts email' do
    user = Factory.create(:passive_user)
    podcast = Factory.create(:parsed_podcast, :owner_email => user.email)
    user.generate_reset_password_code
    lambda { UserMailer.deliver_claim_podcast(user, podcast) }.should change { ActionMailer::Base.deliveries.size }.by(1)
    ActionMailer::Base.deliveries.first.to_addrs.map { |to| to.to_s }.should == ['kfaaborg@limewire.com'] #[user.email]
    ActionMailer::Base.deliveries.first.subject.should == "#{podcast.title} added to LimeCast"
